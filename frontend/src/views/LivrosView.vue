...
async function save() {
  ...
  if (validationMessage) {
    toast.value = { message: validationMessage, type: 'error' }
    return
  }
...
function validateForm(): string | null {
  if (!form.value.titulo.trim() || !form.value.editora.trim() || !form.value.anoPublicacao.trim()) {
    return t('livros.camposObrigatorios')
  }

  if (form.value.edicao < 1 || form.value.valor < 0) {
    return t('livros.camposObrigatorios')
  }

  if (!/^\d{4}$/.test(form.value.anoPublicacao)) {
    return t('livros.anoInvalido')
  }

  if (!form.value.autoresCodAu.size || !form.value.assuntosCodAs.size) {
    return t('livros.selecaoObrigatoria')
  }
...
watch(
  () => form.value.anoPublicacao,
  (value) => {
    const onlyDigits = value.replace(/\D/g, '').slice(0, 4)
    if (onlyDigits !== value) {
      form.value.anoPublicacao = onlyDigits
    }
  },
)
  ...
  <div class="page-shell">
    ...
    >
      <form class="space-y-5" @submit.prevent="save">
        ...
        <BaseInput :label="t('livros.valor')" v-model.number="form.valor" type="number" step="0.01" min="0" required>
          <template #leading>
            <BadgeDollarSign />
          </template>
        </BaseInput>
